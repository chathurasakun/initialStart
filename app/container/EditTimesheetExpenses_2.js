...
class EditTimesheetExpenses_2 extends Component {

    constructor(props) {
        super(props);
        this.state = {
            images: this.props.expenseItem.expensesImageDTOs,
    ...
    addTo = () => {
        if (this.state.amount !== '' && this.state.description !== '' && this.state.images.length !== 0) {
            if (this.props.addNew) {
                let imageSet = this.state.images.slice();
                for (let k in imageSet) {
                    if (imageSet[k].id === undefined) {
                        imageSet[k].image = imageSet[k].uri;
                        imageSet[k].imageData = imageSet[k].data;
                        imageSet[k].status = '7';
                    }
                }
                const expenseObj = {
                    id: (this.state.id !== '') ? this.state.id : Math.random(),
                    expensesImageDTOs: imageSet,
                    amount: this.state.amount,
                    description: this.state.description,
                    unit: 'CAD',
                    status: '7'
                }
                EventRegister.emit('myCustomEvent20', expenseObj);
            }
            else {
                let imageSet = this.state.images.slice();
                for (let k in imageSet) {
...
export default EditTimesheetExpenses_2;